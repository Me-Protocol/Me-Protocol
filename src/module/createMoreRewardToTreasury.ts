...
import { executeCreateMoreRewardToTreasuryWorkflow } from "../helpers/basicOperationHelpers";

/**
 * Creates more rewards to treasury.
 *
 * This function handles user authentication and executes the create more reward
 * to treasury workflow through a relay service.
 *
...
export async function createMoreRewardToTreasuryFN({
  ...
  try {
    ...
    // Execute the create more reward to treasury workflow
    const result = await executeCreateMoreRewardToTreasuryWorkflow({
      signer,
      userInfo,
      rewardAddress,
      amount,
    ...
    console.error("Create more reward to treasury failed:", errorMessage);
