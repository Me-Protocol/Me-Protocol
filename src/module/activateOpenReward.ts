...
import { executeActivateOpenRewardWorkflow } from "../helpers/basicOperationHelpers";

/**
 * Activates an open reward.
 *
 * This function handles user authentication and executes the activate open reward
 ...
 * @throws Error if authentication, activation, or transaction execution fails
 */
export async function activateOpenRewardFN({
  ...
  pk,
  hedera,
}: ActivateOpenRewardProps): Promise<TransactionResult> {
  // Input validation
  ...
  try {
    ...
    // Execute the activate open reward workflow
    const result = await executeActivateOpenRewardWorkflow({
      signer,
      userInfo,
      rewardAddress,
      meApiKey,
    ...
    console.error("Activate open reward failed:", errorMessage);
