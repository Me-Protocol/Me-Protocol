import { createWeb3 } from "../lib/web3";
import { CreateRewardProps } from "../lib/types";
import { TransactionResult } from "../helpers/types";
import { handleUserAuthentication } from "../helpers/authentication";
import { executeCreateRewardWorkflow } from "../helpers/basicOperationHelpers";

/**
 * Creates a new reward token.
 *
 * This function handles user authentication and executes the create reward
 * workflow through a relay service.
 *
 * @param params - Configuration object containing all required parameters
 * @returns Promise resolving to transaction result with taskId
 * @throws Error if authentication, creation, or transaction execution fails
 */
export async function createRewardFN({
  email,
  magic,
  name,
  symbol,
  descriptionLink,
  totalSupply,
  setLoading,
  meApiKey,
  reqURL,
  costPayerId,
  setError,
  GELATO_API_KEY,
  debug,
  OPEN_REWARD_DIAMOND,
  JSON_RPC_URL,
  CHAIN_ID,
  pk,
  hedera,
}: CreateRewardProps): Promise<TransactionResult> {
  // Input validation
  if (!email || !name || !symbol || !totalSupply) {
    throw new Error("Missing required parameters: email, name, symbol, or totalSupply");
  }

  setLoading(true);

  try {
    const magicWeb3 = await createWeb3(magic);

    if (!magicWeb3) {
      throw new Error("Failed to create web3 instance");
    }

    // Handle user authentication and setup
    const { signer, userInfo } = await handleUserAuthentication(magic, magicWeb3, email);

    // Execute the create reward workflow
    const result = await executeCreateRewardWorkflow({
      signer,
      userInfo,
      name,
      symbol,
      descriptionLink,
      totalSupply,
      meApiKey,
      reqURL,
      GELATO_API_KEY,
      JSON_RPC_URL,
      CHAIN_ID,
      OPEN_REWARD_DIAMOND,
      costPayerId,
      debug,
      pk,
      hedera,
    });

    return result;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : "An unknown error occurred";
    console.error("Create reward failed:", errorMessage);

    setError(error);
    throw error;
  } finally {
    setLoading(false);

    // Always logout user after operation
    try {
      await magic.user.logout();
    } catch (logoutError) {
      console.warn("Failed to logout user:", logoutError);
    }
  }
}
