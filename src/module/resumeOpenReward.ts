...
import { executeResumeOpenRewardWorkflow } from "../helpers/basicOperationHelpers";

/**
 * Resumes an open reward.
 *
 * This function handles user authentication and executes the resume open reward
 ...
 * @throws Error if authentication, resume operation, or transaction execution fails
 */
export async function resumeOpenRewardFN({
  ...
  pk,
  hedera,
}: ResumeOpenRewardProps): Promise<TransactionResult> {
  // Input validation
  ...
  try {
    ...
    // Execute the resume open reward workflow
    const result = await executeResumeOpenRewardWorkflow({
      signer,
      userInfo,
      rewardAddress,
      meApiKey,
    ...
    console.error("Resume open reward failed:", errorMessage);
